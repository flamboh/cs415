...
int main(int argc, char* argv[]) {
     ...
     */
    int *numbers = (int*)malloc(NUM_WORKERS * sizeof(int));
    if (numbers == NULL) {
        perror("malloc");
        exit(EXIT_FAILURE);
    }
     ...
     */
    for (int i = 0; i < NUM_WORKERS; i++) {
        if (pthread_create(&thread_ids[i], NULL, simulate_work, &numbers[i]) != 0) {
     ...
     */
    printf("Counter: %d\n", counter);
    printf("NUM_WORKERS: %d, INCREMENTS_PER_THREAD: %d\n", NUM_WORKERS, INCREMENTS_PER_THREAD);
    if (counter != NUM_WORKERS * INCREMENTS_PER_THREAD) {
        printf("Warning: counter value does not match expected value.\n");
    }
    else {
        printf("Success: counter value matches expected value.\n");
    }
...
void* simulate_work(void* arg) {
    ...
    snprintf(message, MESSAGE_SIZE, "Thread %d finished work. Counter is now %d.\n", id, counter);
